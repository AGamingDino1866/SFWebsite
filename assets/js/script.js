...
const injectNavyTheme = () => {
  ...
  style.textContent = `
    :root{--ink:#173d31!important;--muted:#607066!important;--green:#173d31!important;--green-2:#2f7a61!important;--rose:#d56b91!important;--rose-dark:#8d315b!important;--gold:#f1b84b!important;--sky:#79b7c8!important;--blush:#f8e1e9!important;--cream:#f7f8ef!important;--paper:#ffffff!important;--card:#ffffff!important;--line:rgba(23,61,49,.16)!important;--shadow:0 24px 70px rgba(23,61,49,.14)!important;--soft-shadow:0 16px 40px rgba(23,61,49,.1)!important}
    body,.centered-header,.site-header,.auth-box,.result-grid div,.receipt-note,.note-band:not(.force-navy){background:#f7f8ef!important;color:#173d31!important}
    .soft-band{background:#eef3eb!important;color:#173d31!important}
    .brand-mark,.button.primary,.button:not(.secondary):not(.light),.nav-links a.active,.nav-links a:hover,.scholarship-card.featured,.cta-panel,.note-band,.step span,.google-dot,.chat-row.user .bubble{background:#173d31!important;color:#ffffff!important}
    .button.secondary,.button.light,.google-button,.account-actions .button,.limit-pill,input,select,textarea,.surface-card,.feature-card,.university-card,.portal-card,.scholarship-card,.ai-panel,.tip-card,.lookup-card,.result-card,.contact-card,.auth-panel,.auth-card,.admin-panel,.criteria-card,.docs-card,.income-card{background:#ffffff!important;color:#173d31!important;border-color:rgba(23,61,49,.16)!important}
    .hero-card,.auth-hero,.contact-hero,.status-hero,.eligibility-hero,.ai-banner{background:linear-gradient(135deg,rgba(23,61,49,.92),rgba(23,61,49,.62)),url("assets/education-hero.png") center/cover!important;color:#ffffff!important}
    ...
    .eyebrow{color:#d56b91!important}.hero-card .eyebrow{color:#ffdbe8!important}
    .surface-card p,.feature-card p,.university-card p,.portal-card p,.scholarship-card p,.scholarship-card li,.tip-card p,.lookup-form small,.result-message,.receipt-note,.contact-card p,.auth-panel p,.criteria-card p,.docs-card p,.income-card p{color:#607066!important}
  `;
...
const injectMobileNavFix = () => {
  ...
  style.textContent = `
    ...
    @media (max-width: 920px) {
      .nav-toggle{display:block!important;flex:0 0 auto!important}.centered-nav{justify-content:space-between!important;gap:12px!important}.centered-nav .logo-only{position:static!important;transform:none!important}.centered-nav .nav-links{position:fixed!important;inset:76px 12px auto 12px!important;display:none!important;flex-direction:column!important;align-items:stretch!important;gap:6px!important;padding:12px!important;border-radius:14px!important;background:rgba(255,255,255,.96)!important;box-shadow:0 24px 70px rgba(23,61,49,.22)!important;z-index:90!important;white-space:normal!important}.centered-nav .nav-links a{width:100%!important;text-align:left!important;padding:13px 14px!important}body.nav-open .centered-nav .nav-links{display:flex!important}
    }
